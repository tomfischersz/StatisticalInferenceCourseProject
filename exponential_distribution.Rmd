...
```
```{r ref.label="fig_hist_2", echo=FALSE, eval=TRUE}
```
...
Finally we want to examine, if the distribution of our simulation of 1000 averages of 40 random exponentials is approximately normal. In figure \ref{fig:fig_hist_2} we plot the histogram of our simulation data together with the theoretical normal distribution we would except according to the CLT. In figure \ref{fig:fig_qqnorm} we plot a Q-Q (Quantile-Quantile) Plot, where we compare the theoretical quantiles with observed quantiles.

```{r plot_hist_2, echo=FALSE,fig.height=2.8,fig.width=5.5, fig.show='hold',fig.align='center',fig.cap="\\label{fig:fig_hist_2}Sampling distribution of the sample mean. The red line shows the standard normal pdf we would except according the CLT."}
plot(hist_2)
```
...
Both figure \ref{fig:fig_qqnorm} , showing a nearly linear plot and figure \ref{fig:fig_hist_2} , showing a density distribution that is quite normal, let us conclude that in fact our simulated distribution of 1000 averages of 40 random exponential variables follows quite good a normal gaussian distribution. That is what we expected from the Central Limit Theorem.  
...
### 4. Calculating theoretical and actual variance: {#Appendix_4}
...
### 5. Histogram 2: {#Appendix_5}
```{r fig_hist_2, eval=FALSE}
